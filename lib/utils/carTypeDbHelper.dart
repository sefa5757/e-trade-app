...
class CarTypeDbHelper {
  static final CarTypeDbHelper _dbHelper = CarTypeDbHelper._internal();
  String tbl = "cartypes";
  String colId = "id";
  String colName = "name";

  CarTypeDbHelper._internal();

  factory CarTypeDbHelper() {
    return _dbHelper;
  }
  ...
  Future<int> insertEntity(CarType entity) async {
  ...
  Future<List> getEntities() async {
    List<CarType> entityList = <CarType>[];
    var db = await this.db;
    var result =
        await db!.rawQuery("SELECT * FROM $tbl");
    for (var element in result) {
      entityList.add(CarType.fromObject(element));
    }
    return entityList;
  }
  ...
  Future<int> updateEntity(CarType entity) async {
    var db = await this.db;
